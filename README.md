...
> 🔀 **ถ้าเปิด `fallback_enabled=true` ไว้**: ระหว่างรอเติมสต็อก บอทจะลองเลื่อนไป
> option อื่นในช่วง `max_fallback_steps` ที่**มีสต็อกอยู่แล้ว**ทันที — ไม่ต้องรอ
> ตัวเดิมกลับมา (ตัวเดิมยังได้ priority ถ้ากลับมามีของก่อน)
...
> 🔀 Fallback นี้ทำงาน 2 จุด: (1) เลือก variant ครั้งแรกตอนเข้าหน้าสินค้า
> (2) **ระหว่างรอเติมสต็อก** — ถ้า option อื่นในช่วงที่อนุญาตมีของก่อน
> บอทจะเลื่อนไปซื้อตัวนั้นทันที (ดู Restock Wait)
